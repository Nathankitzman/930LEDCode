...
//Runs function specified                                         /// Ok, put it this way, loop runs preset() over and over.
void loop() {
  preset();
}
...
void preset() {
  ...
  colorWipe(green, 100);
  delay(1000);
  for (int i=0; i< 2; i++) {                                                        /// PUT SPACES BETWEEN = and COMPARISON SIGNS (please)
    rainbowWheel(50);
  }
  for (int i=0; i< 50; i++) {
...
void movingSegment(uint32_t color, int segLength, int wait) {
  ...
  for(int i=0; i <= strip.numPixels(); i++) {                                                  ///  ADD EMPTY LINES AFTER LINES OF CODE
...
void movingPong(uint32_t color, int segLength, int wait) {
  //Changes position one over
  for(int i=0; i <= strip.numPixels()-segLength; i++) {                                         ///  ADD EMPTY LINES AFTER LINES OF CODE
    //Writes the length of the strip
    for(int j=0; j < segLength; j++) {
      strip.setPixelColor(j + i, color);
    }
...
void rainbowWheel(int wait) {
  //Splits hue spectrum equally to all pixels
  uint16_t dif = 65536/strip.numPixels();                                         ///  ADD EMPTY LINES AFTER LINES OF CODE
  ...
  for(int k=0; k < strip.numPixels(); k++) {
    ...
    for(int j=0; j < dif; j+=1000) {
      ...
      for(int i=0; i < strip.numPixels(); i++) {
        //Wraps pattern to create a "wheel"
        if (i-k >= 0) {
          strip.setPixelColor(i-k, strip.ColorHSV((i*dif)+j, 255, 255));                          /// A lot of opportunity to add spacing here. Ex: (i*dif)+j ---> (i * dif) + j
        } else {
          strip.setPixelColor((i-k)+strip.numPixels(), strip.ColorHSV((i*dif)+j, 255, 255));
        }
      }
